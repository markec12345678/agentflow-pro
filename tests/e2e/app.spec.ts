/**
 * AgentFlow Pro - E2E Tests (OUTDATED - IGNORED)
 *
 * This spec is excluded via playwright.config.ts testIgnore because it references
 * non-existent routes and data-testids:
 * - /auth/register, /auth/login, /auth/forgot-password (actual: /register, /login, /forgot-password)
 * - /tourism/workflows, /tourism/multilang, /booking/*, /reviews/*, /billing/* (do not exist)
 * - data-testid attributes (email-input, plan-select, etc.) not present on actual pages
 *
 * Use auth.spec.ts, smoke-checklist.spec.ts, tourism.spec.ts for current E2E coverage.
 * To re-enable: update routes, add data-testids, or rewrite to match actual app structure.
 */
