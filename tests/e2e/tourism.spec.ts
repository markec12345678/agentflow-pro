...
authTest.describe("Tourism Hub - Authenticated", () => {
  authTest("Tourism generate page loads with prompts", async ({ page, auth: _auth }, testInfo) => {
  ...
  authTest("Email Generator loads", async ({ page, auth: _auth }, testInfo) => {
    testInfo.setTimeout(30000);
  ...
  authTest("Multi-Language Translator loads", async ({ page, auth: _auth }, testInfo) => {
    testInfo.setTimeout(30000);
