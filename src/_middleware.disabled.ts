...
const CUSTOM_CORS_RULES: Record<
  string,
  { origins: string[]; methods: string[]; headers?: string[] }
> = {
  "/api/public": {
    origins: ["*"],
    methods: ["GET", "POST"],
...
export async function middleware(request: NextRequest) {
  ...
  if (isApiRoute) {
    ...
    if (customRules) {
      ...
      const allowedOrigin = customRules.origins.includes("*")
        ? "*"
        : customRules.origins.find((o) => o === origin) ||
          customRules.origins[0];

      response.headers.set("Access-Control-Allow-Origin", allowedOrigin);
      response.headers.set(
        "Access-Control-Allow-Methods",
        customRules.methods.join(", "),
      );
      response.headers.set(
        "Access-Control-Allow-Headers",
        (customRules.headers || ALLOWED_HEADERS).join(", "),
      );
  ...
  const cspDirectives = developmentMode
    ? [
        ...
        "connect-src 'self' https://*.sentry.io https://*.vercel.app http://localhost:*",
