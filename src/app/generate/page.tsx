...
function GenerateWizard() {
  ...
  useEffect(() => {
    ...
    if (template && name && loc && !auto) setStep(2);
  }, [searchParams]);

  // Auto-generate za seasonal-campaign&auto=1
  useEffect(() => {
    const template = searchParams.get("template");
    const auto = searchParams.get("auto") === "1";
    if (template !== "seasonal-campaign" || !auto || autoGenerated) return;
    ...
    const d = now.getDate();
    let season = "Poletje";
    if (m === 11 && d >= 20) season = "Zima";
    else if (m === 12) season = "Božič & novo leto";
    else if (m >= 2 && m <= 3) season = "Pomlad";
    else if (m >= 5 && m <= 6) season = "Poletje";
    else if (m >= 9 && m <= 10) season = "Jesen";

    const autoFields = {
      name: p?.name ?? "Nastanitev",
      location: p?.location ?? "Slovenija",
      season,
      offer: "",
      cta: "Rezervirajte zdaj",
    };
    ...
    const topic = [template, autoFields.name, autoFields.location, autoFields.season].filter(Boolean).join(" - ");
    fetch("/api/generate-content", {
      ...
      .then((data) => {
        ...
        setResult(text || getMockResult(template as string, autoFields.name, autoFields.location, "sl"));
      })
      .catch(() => setResult(getMockResult(template as string, autoFields.name, autoFields.location, "sl")))
      .finally(() => setGenerating(false));
  }, [searchParams, properties, autoGenerated]);
