...
function LoginForm() {
  ...
  const [error, setError] = useState(() => {
    ...
    if (err === "CredentialsSignin") return "Napačen email ali geslo";
    if (err === "OAuthSignin") return "Napaka pri Google prijavi. Poskusite ponovno.";
    if (err === "OAuthCallback") return "Google se ni mogel povezati. Preverite nastavitve.";
    if (err === "OAuthAccountNotLinked") return "Ta Google račun je že povezan z drugim uporabnikom.";
    if (err === "AccessDenied") return "Dostop zavrnjen. Omogočite dostop v Google.";
    if (errorDesc) return decodeURIComponent(errorDesc);
    if (err) return "Prišlo je do napake. Poskusite ponovno.";
    return "";
  });
  const [loading, setLoading] = useState(false);
    ...
    <main className="flex min-h-screen items-center justify-center p-8 relative z-40">
      <div className="w-full max-w-sm rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 relative z-50">
        ...
        {error && (
          <div className="mb-4 relative">
            ...
            <button
              type="button"
              onClick={() => setError("")}
              className="absolute top-1 right-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
        ...
        {googleEnabled && (
          <>
            ...
            {error && error.includes("Google") && (
              <p className="mb-4 text-xs text-center text-amber-700 dark:text-amber-300">
                💡 <strong>Nasvet:</strong> Če Google prijava ne deluje, preverite ali so GOOGLE_CLIENT_ID in GOOGLE_CLIENT_SECRET pravilno nastavljeni v .env datoteki.
              </p>
            )}
        ...
        <p className="mt-4 text-center text-sm text-gray-600 dark:text-gray-400">
          Nimate računa? <Link href="/register" className="text-indigo-600 hover:underline">Registracija</Link>
        </p>
