...
async function sendPreArrivalReminder(booking: {
  ...
  console.log(`✅ Pre-arrival reminder sent to ${booking.guestEmail}`);
}
...
async function sendPostStayEmail(booking: {
  ...
  console.log(`✅ Post-stay email sent to ${booking.guestEmail}`);
}
...
async function sendPaymentConfirmation(payment: {
  ...
  console.log(`✅ Payment confirmation sent to ${payment.guestEmail}`);
}
...
async function sendCancellationConfirmation(cancellation: {
  ...
  console.log(`✅ Cancellation confirmation sent to ${cancellation.guestEmail}`);
}
...
function findTemplateCategory(templateId: string) {
  ...
  if (category) {
    console.log(`✅ Template "${templateId}" belongs to category: ${category}`);
  } else {
    console.log(`❌ Template "${templateId}" not found in any category`);
  }
...
function getDefaultTemplateForCategory(category: string) {
  ...
  console.log(`📧 Default template for "${category}": ${defaultTemplate}`);
...
async function sendBatchEmails(
  ...
  console.log(`📧 Sending ${recipients.length} emails using template: ${template.name}`);
  ...
  console.log(`✅ Batch send complete: ${successCount} succeeded, ${failCount} failed`);
...
async function handleEmailAutomation(trigger: EmailAutomationTrigger) {
  console.log(`🔔 Email automation triggered: ${trigger.eventType}`);
  ...
  switch (trigger.eventType) {
    ...
    default:
      console.log(`⚠️ Unknown event type: ${trigger.eventType}`);
  }
}
